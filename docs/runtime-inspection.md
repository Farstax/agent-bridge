...
- `sessions`: existence and creation time for the current conversation only; native session identifiers are never returned. Codex reports runtime `acp`; session existence comes only from `acp_session_bindings`. Legacy Codex session columns are not projected as rollback state.
