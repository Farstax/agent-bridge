...
export async function discoverAcpProviderConfig(input: AcpProviderConfigDiscoveryInput): Promise<AcpProviderConfigDiscoveryResult> {
  ...
  const request: ProviderInvocationRequest = {
    ...
    outputDir: null, effort: null, toolMode: "default",
  };
  ...
  Object.assign(contextEnv, providerEnv);
  const runId = `config-discovery:${randomUUID()}`;
  const options: CliOptions = { bot: input.bot, timeoutMs: CONFIG_DISCOVERY_TIMEOUT_MS, idleTimeoutMs: CONFIG_DISCOVERY_TIMEOUT_MS, contextEnv, chatId: runId };
  const result = await runSupervisedStdioSession(
    runtime.executable, [...runtime.args], input.cwd, options,
