...
function scope(db: Database.Database, env: Env) {
  ...
  if (runId && hasTable(db, "bridge_runs")) {
    ...
    chatKey ||= text(run?.chat_id, 240); provider = pid(run?.bot);
  }
  if (runId && hasTable(db, "execution_locks")) {
    ...
    surface ||= text(lock?.surface); chatKey ||= text(lock?.chat_key, 240);
  }
...
function runReason(db: Database.Database, runId: string, status: string) {
  ...
  let payload: Row = {}; try { payload = JSON.parse(String(row.payload_json ?? "{}")) as Row; } catch {}
...
function execution(db: Database.Database, s: ReturnType<typeof scope>) {
  ...
  return {
    status: "ready", reasonCode: null, currentRunId: s.runId,
...
function routines(db: Database.Database, s: ReturnType<typeof scope>, env: Env) {
  ...
  for (const row of rows) {
    ...
    let r: Row; try { r = JSON.parse(String(row.value)) as Row; } catch { continue; }
    if (r.surfaceIdentity !== s.surface || r.chatKey !== s.chatKey || (owner && r.ownerKey !== owner)) continue;
    const id = text(r.id, 100); if (!id) continue;
...
function withOptionalDb<T>(path: string | null, mainPath: string, main: Database.Database, unavailable: T, read: (db: Database.Database) => T): T {
  ...
  try { const db = openRo(path); try { return read(db); } finally { db.close(); } } catch { return unavailable; }
}
...
function autonomy(main: Database.Database, mainPath: string, env: Env) {
  return withOptionalDb(text(env.AGENT_BRIDGE_AUTONOMY_DB_PATH, 500), mainPath, main, { status: "unknown", reasonCode: "autonomy_db_unavailable", goals: [] as Row[] }, (db) => {
    ...
    return { status: "ready", reasonCode: null, goals: goals.map((goal) => {
      const goalId = String(goal.goal_id); const wake = receipts.find((item) => { try { return (JSON.parse(String(item.payload_json ?? "{}")) as Row).goalId === goalId; } catch { return false; } });
      let wakeReason: string | null = null; if (wake) try { wakeReason = code((JSON.parse(String(wake.payload_json ?? "{}")) as Row).reason, "unclassified"); } catch {}
...
function health(main: Database.Database, mainPath: string, env: Env) {
  ...
  return withOptionalDb(text(env.HEALTH_DB_PATH, 500), mainPath, main, { enabled: true, status: null, reasonCode: "health_db_unavailable", freshnessSeconds, nonGreen: [] as Row[], stalePluginNames: [] as string[], missingPluginNames: [] as string[] }, (db) => {
    ...
    const byName = new Map(rows.map((row) => [String(row.plugin_name), row])); const stale: string[] = [], missing: string[] = [], fresh: Array<{pluginName:string;status:"green"|"amber"|"red";savedAt:string}> = []; const now = Math.floor(Date.now()/1000);
    for (const name of names) { const row = byName.get(name); if (!row) { missing.push(name); continue; } const saved = Number(row.saved_at); if (!Number.isFinite(saved) || now-saved>freshnessSeconds) { stale.push(name); continue; } try { const report = JSON.parse(String(row.report_json)) as Row; if (report.status!=="green"&&report.status!=="amber"&&report.status!=="red") { missing.push(name); continue; } fresh.push({pluginName:name,status:report.status,savedAt:new Date(saved*1000).toISOString()}); } catch { missing.push(name); } }
...
function providers(s: ReturnType<typeof scope>, env: Env, commit: string | null) {
  const bots = loadBotsConfig(env); const path = env.AGENT_BRIDGE_PROVIDER_QUALIFICATION_PATH?.trim() || qualificationEvidencePath(env.HOME?.trim() || homedir());
  let evidence: ReturnType<typeof readQualificationEvidence> | null = null, evidenceReason: string | null = null; try { evidence = readQualificationEvidence(path); } catch { evidenceReason = "qualification_evidence_unreadable"; }
  return getProviderAdapters().map((adapter) => { const record = evidence?.providers[adapter.id]; const key = (adapter.id === "agy" ? "antigravity" : adapter.id) as keyof typeof bots; return { id: adapter.id, displayName: adapter.displayName, selected: s.provider === adapter.id, availability: s.provider === adapter.id ? "available" : "unknown", availabilityReasonCode: s.provider === adapter.id ? null : "not_live_probed", authentication: "unknown", defaultModel: bots[key].modelPreference[0] ?? null, qualification: !record ? { status: "unknown", reasonCode: evidenceReason ?? "no_qualification_evidence" } : record.contractVersion !== PROVIDER_CONTRACT_VERSION ? { status: "unqualified", reasonCode: "provider_contract_changed", lastResult: record.overall, providerVersion: text(record.providerVersion,80), contractVersion: record.contractVersion, qualifiedAt: text(record.qualifiedAt,60) } : { status: "unknown", reasonCode: "runtime_version_unobserved", lastResult: record.overall, providerVersion: text(record.providerVersion,80), contractVersion: record.contractVersion, qualifiedAt: text(record.qualifiedAt,60), bridgeCommitMatches: Boolean(commit && record.bridgeCommit===commit) } }; });
}

function skills(env: Env) {
  const home = getSharedSkillsHomeDir({HOME:env.HOME,SHARED_MEMORY_HOME:env.SHARED_MEMORY_HOME}, homedir()); const paths = resolveSkillPaths(home); let installed: string[] = [], installedStatus: "ready"|"unknown"|"unavailable" = "unavailable", installedReasonCode: string|null = "skill_index_missing";
  if (existsSync(paths.lockfilePath)) try { const value = JSON.parse(readFileSync(paths.lockfilePath,"utf8")) as {skills?:Record<string,unknown>}; installed = Object.keys(value.skills??{}).sort().slice(0,24); installedStatus="ready"; installedReasonCode=null; } catch { installedStatus="unknown"; installedReasonCode="skill_index_unreadable"; }
  let bundled: string[] = []; try { bundled = listLocalCatalog(ROOT).map((x)=>x.name).slice(0,24); } catch {}
...
function capabilityIndex(s: ReturnType<typeof scope>, env: Env, h: ReturnType<typeof health>, sk: ReturnType<typeof skills>, a: ReturnType<typeof autonomy>, ps: ReturnType<typeof providers>) {
  const inspect = join(ROOT,"bin","agent-bridge-inspect"), context = join(ROOT,"bin","agent-bridge-context"), routine = env.AGENT_BRIDGE_ROUTINES_COMMAND?.trim() || join(ROOT,"bin","agent-bridge-routines"); const scoped=Boolean(s.chatKey&&s.surface); const cap=(id:string,status:string,reasonCode:string|null,scope:string,risk:string,authorityRequired:string,iface:string)=>({id,owner:"agent-bridge",status,reasonCode,scope,risk,authorityRequired,interface:iface});
  return [cap("runtime-inspection","ready",null,"runtime","read-only","none",`${inspect} --json`),cap("retained-context",scoped?"ready":"unavailable",scoped?null:"conversation_scope_unavailable","conversation","read-only","current conversation or proved owner",context),cap("advisor",env.AGENT_BRIDGE_ADVISOR_COMMAND&&env.AGENT_BRIDGE_ADVISOR_CAPABILITY?"ready":"unavailable",env.AGENT_BRIDGE_ADVISOR_COMMAND&&env.AGENT_BRIDGE_ADVISOR_CAPABILITY?null:"turn_capability_unavailable","turn","read-only-advice","turn capability",env.AGENT_BRIDGE_ADVISOR_COMMAND?.trim()||"agent-bridge-advisor"),cap("scheduled-routines",scoped&&existsSync(routine)?"ready":"unavailable",!scoped?"conversation_scope_unavailable":existsSync(routine)?null:"routine_command_unavailable","conversation","state-change","authenticated owner",routine),cap("autonomous-work",s.surface==="telegram:interactive"&&a.status==="ready"?"ready":"unavailable",s.surface!=="telegram:interactive"?"surface_not_supported":a.status==="ready"?null:a.reasonCode,"goal","execution","authenticated owner","first-class autonomy"),cap("health-investigation",h.enabled?"ready":"unavailable",h.enabled?null:"health_monitor_disabled","runtime","read-only","none","runtime inspector health projection"),cap("installed-skills",sk.installedStatus,sk.installedReasonCode,"runtime","read-only","none",sk.root),cap("chat-surfaces","ready",null,"runtime","read-only","none","telegram,discord"),cap("provider-execution",ps.some((p)=>p.selected)?"ready":"unknown",ps.some((p)=>p.selected)?null:"no_current_run_context","run","execution","existing bridge authority","native provider CLI")];
}

export function buildAgentBridgeInspection(env: Env = process.env) {
  const mainPath = dbPath(env), db = openRo(mainPath); try {
    const schema = Number(db.pragma("user_version",{simple:true})), s=scope(db,env), commit=text(env.AGENT_BRIDGE_COMMIT??env.BRIDGE_COMMIT??env.BRIDGE_RELEASE_COMMIT,120), ex=execution(db,s), ss=sessions(db,s), rs=routines(db,s,env), a=autonomy(db,mainPath,env), h=health(db,mainPath,env), sk=skills(env), ps=providers(s,env,commit), caps=capabilityIndex(s,env,h,sk,a,ps);
    let version: string|null=null; try { version=text((JSON.parse(readFileSync(join(ROOT,"package.json"),"utf8")) as Row).version,40); } catch {}
    const currentSurface=s.surface?.startsWith("telegram:")?"telegram":s.surface?.startsWith("discord:")?"discord":null;
    return { schemaVersion:1, generatedAt:new Date().toISOString(), runtime:{packageVersion:version,commit,service:{status:s.runId&&ex.activeRuns.some((r)=>r.runId===s.runId)?"ready":"unknown",reasonCode:s.runId?null:"no_current_run_context"},database:{status:schema===CURRENT_SCHEMA_VERSION?"ready":"unavailable",schemaVersion:schema,expectedSchemaVersion:CURRENT_SCHEMA_VERSION},degradations:[schema===CURRENT_SCHEMA_VERSION?null:"database_schema_mismatch",h.status==="red"?"health_red":h.status==="amber"?"health_amber":null,sk.installedStatus==="unknown"?"skill_index_unreadable":null].filter(Boolean)}, providers:ps, execution:ex, sessions:ss, scheduledRoutines:rs, autonomy:a, surfaces:["telegram","discord"].map((id)=>({id,status:currentSurface===id?"ready":"unknown",reasonCode:currentSurface===id?null:"not_current_surface",current:currentSurface===id})), health:h, knowledge:{skills:{installedStatus:sk.installedStatus,installedReasonCode:sk.installedReasonCode,installed:sk.installed,bundled:sk.bundled},retainedContext:{interface:join(ROOT,"bin","agent-bridge-context"),scopeKnown:Boolean(s.chatKey&&s.surface)},scheduledRoutines:{interface:env.AGENT_BRIDGE_ROUTINES_COMMAND?.trim()||join(ROOT,"bin","agent-bridge-routines")},advisor:{availableForTurn:Boolean(env.AGENT_BRIDGE_ADVISOR_COMMAND&&env.AGENT_BRIDGE_ADVISOR_CAPABILITY)},supportedProviders:getProviderAdapters().map((x)=>x.id),supportedChatSurfaces:["telegram","discord"]}, capabilities:caps };
  } finally { db.close(); }
}
...
export function renderAgentBridgeInspection(args: string[], env: Env = process.env): string {
  ...
  const full=buildAgentBridgeInspection(env); const view=args[0]==="capabilities"?{schemaVersion:full.schemaVersion,generatedAt:full.generatedAt,runtime:{packageVersion:full.runtime.packageVersion,commit:full.runtime.commit},capabilities:full.capabilities}:full; const out=JSON.stringify(view); if(out.length>MAX_INSPECTION_OUTPUT_CHARS) throw new Error(`runtime inspector output exceeded ${MAX_INSPECTION_OUTPUT_CHARS} characters`); return out;
}
