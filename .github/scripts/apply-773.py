...
engine_pattern = "does not inject context into a resumed native session|handoff_once injects on the first turn when no native session exists|injects Soul and the active model once on handoff|re-seeds baseline fresh-session context after reset without restoring deleted history|injects context on invalid-session retry under handoff_once|falls back to the next model in preference list"
run("npx", "vitest", "run", "test/engine.test.ts", "-t", engine_pattern)
run("npx", "vitest", "run", "test/issue637FallbackContinuationIntent.test.ts", "test/manualCliSwitchHandoff.test.ts", "test/executionLaneCorrectness.test.ts")
