...
describe("immutable release staging", () => {
  it("stages and validates an exact archive into a commit-addressed immutable directory", () => {
    ...
    const provenancePath = join(releaseRoot, `.${COMMIT}.staging-provenance.json`);
    chmodSync(provenancePath, 0o644);
    writeFileSync(provenancePath, JSON.stringify({ ...provenance, release_stage_sha256: "f".repeat(64) }) + "\n", { mode: 0o444 });
    chmodSync(provenancePath, 0o444);
