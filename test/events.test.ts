...
describe("RunView reducer", () => {
  ...
  it("acp.retained does not change delivered text/status — Telegram/Discord never redeliver it", async () => {
    const { reduce } = await import("../src/events/reducer.js");
    const { type } = await import("../src/events/types.js");
    ...
    const view = reduce([
      ...
      type.acpRetained({
        ...base,
        sessionMode: "load",
        events: [{ kind: "session_update", channel: "replay", notification: { update: { sessionUpdate: "agent_message_chunk", content: { type: "text", text: "replayed history" } } } }],
      }),
