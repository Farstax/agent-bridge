...
describe("source voice STT install", () => {
  it("converges the OSS-owned STT helper before enabling source-installed services", () => {
    ...
    const converge = script.indexOf('sudo /usr/bin/env AGENT_BRIDGE_STT_ROOT="/opt/agent-bridge/host-components/voice-stt"');
    const installer = script.indexOf('/bin/bash "${REPO_DIR}/scripts/install-voice-stt.sh"', converge);
    const enable = script.indexOf("sudo systemctl enable ${UNITS_TO_ENABLE}");
    expect(converge).toBeGreaterThan(-1);
    expect(installer).toBeGreaterThan(converge);
    expect(enable).toBeGreaterThan(installer);
  });
