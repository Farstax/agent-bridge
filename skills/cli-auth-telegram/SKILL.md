...
2. Check whether it is already authenticated by testing for its credential file before doing anything else:
   ...
   If the file exists and is non-empty, treat that as credential presence rather than proof of usability. For Claude, after resolving the binary in step 3, run `claude auth status --json` under the same exclusive Agent Bridge credential lock described below; only exit code 0 is evidence that the current credential state is logged in. If Claude was explicitly reported as needing re-authentication, continue the re-auth flow instead of treating the file alone as connected. For the other file-backed account flows, tell the user it looks already connected and ask whether they want to re-authenticate anyway before proceeding.
...
After the credential file is confirmed present, verify Claude with `<resolved-claude-bin> auth status --json` under the same exclusive Agent Bridge credential lock and require exit code 0 before declaring Claude connected. For the other covered CLIs, retain their provider-specific verification above. Never repeat secret material from the exchange.
