...
2. Confirm the installed provider CLI and version. Do not infer support from another provider.
...
Configure only providers that are installed and natively support the requested MCP.
...
- Other providers: inspect their current native CLI help/documentation and qualify them separately before relying on MCP.
