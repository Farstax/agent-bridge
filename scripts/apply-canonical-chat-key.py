...
def add_tests() -> None:
    (ROOT / "test/canonicalConversationIdentity.test.ts").write_text(r'''import { describe, expect, it } from "vitest";
...
function config(allowedUserIds: ReadonlySet<string>): BridgeConfig {
  const bot = { token: undefined, command: "codex", modelPreference: ["default"] };
  return {
    allowedUserIds,
    serviceEnvFile: null,
    serviceKind: null,
    ...
    asyncEnabled: false,
    dbPath: ":memory:",
    bots: { codex: bot, claude: { ...bot, command: "claude" }, antigravity: { ...bot, command: "agy" } },
  };
}
...
describe("canonical conversation identity", () => {
  it("keeps colliding Discord Snowflakes distinct across Engine-owned conversation state", async () => {
    const max = BigInt(Number.MAX_SAFE_INTEGER);
    const nativeA = "1";
    const nativeB = String(max + 1n);
    const oldAlias = (snowflake: string) => String(Number(BigInt(snowflake) % max));
    expect(oldAlias(nativeA)).toBe(oldAlias(nativeB));
    ...
    try {
      ...
      db.enqueueMsg(surface, nativeA, { prompt: "queued-a", chatId: 1, chatType: "private", userId: 7 });
      db.enqueueMsg(surface, nativeB, { prompt: "queued-b", chatId: 1, chatType: "private", userId: 7 });
      ...
      await (engine.handleUpdate as any)(queueModeUpdate(2), nativeB);
  ...
  it("keeps Telegram private, group, and topic keys unchanged", () => {
    const update = (chatId: number, type: string, threadId?: number): TelegramUpdate => ({
      update_id: 1,
      message: {
        message_id: 1,
        chat: { id: chatId, type },
  ...
  it("persists and reconciles the authoritative Run chatKey rather than delivery coordinates", async () => {
...
def implement() -> None:
    ...
    t, n = re.subn(r'const snowflakeAliases = new Map<string, string>\(\);\n\nclass DiscordEngineClient implements MessagingPlatform \{.*?\n\}\n\n// ── DiscordClient', '// ── DiscordClient', t, flags=re.S)
    if n != 1:
    ...
    t, n = re.subn(
        r'function rememberSnowflakeAlias\(snowflake: string\): number \{.*?\n\}\n\nfunction numericId\(snowflake: string\): number \{\n  return Number\(discordLaneKey\(snowflake\)\);\n\}',
        '''function numericId(snowflake: string): number {\n  // Only update/message IDs pass through this Telegram-shaped numeric field.\n  // Conversation identity and recovery always use the native string chatKey.\n  const value = BigInt(snowflake || "0");\n  return Number(value % BigInt(Number.MAX_SAFE_INTEGER));\n}''',
        t,
        flags=re.S,
    )
    if n != 1:
        raise RuntimeError(f"{p}: numeric transport utility replacement found {n}")
    if any(token in t for token in ('discordLaneKey', 'snowflakeAliases', 'engineClient', 'rememberSnowflakeAlias')):
