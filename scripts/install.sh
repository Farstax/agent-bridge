...
seed_from_env_file() {
  ...
  for key in BRIDGE_ROOT_DIR BRIDGE_PROJECT_DIR BRIDGE_CURRENT_RELEASE_DIR \
              ...
              AGENT_BRIDGE_SENSOR_CONFIG BRIDGE_RUN_INGRESS_SOCKET BRIDGE_RUN_INGRESS_TOKEN \
...
_write_interactive_defaults() {
  local dest="${DEFAULTS_DIR}/agent-bridge-interactive"
  {
    ...
    [[ -n "${AGENT_BRIDGE_SENSOR_CONFIG:-}" ]] && echo "AGENT_BRIDGE_SENSOR_CONFIG=${AGENT_BRIDGE_SENSOR_CONFIG}"
    [[ -n "${BRIDGE_RUN_INGRESS_SOCKET:-}" ]] && echo "BRIDGE_RUN_INGRESS_SOCKET=${BRIDGE_RUN_INGRESS_SOCKET}"
    [[ -n "${BRIDGE_RUN_INGRESS_TOKEN:-}" ]] && echo "BRIDGE_RUN_INGRESS_TOKEN=${BRIDGE_RUN_INGRESS_TOKEN}"
    true
  } | sudo tee "${dest}" > /dev/null
  echo "  wrote ${dest}"
}
